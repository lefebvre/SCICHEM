  ...
  IF( numField == 1 .AND. Request%Shh == 0. )THEN
    ...
    IF( lSaveCoord )THEN
      IF( PrjCoord%type /= field(1)%grid%coord%type )THEN
        xtmp = xFld(1)
        ytmp = yFld(1)
        irv = SWIMcnvCoord( Request%X,Request%Y,PrjCoord,xtmp,ytmp,field(1)%grid%coord )
        xFld(1) = xtmp
        yFld(1) = ytmp
      ELSE
...
ELSE IF( lSaveCoord )THEN
  ...
  IF( PrjCoord%type /= field(ifld)%grid%coord%type )THEN
    xtmp = xFld(ifld)
    ytmp = yFld(ifld)
    irv  = SWIMcnvCoord( Request%X,Request%Y,PrjCoord,xtmp,ytmp,field(ifld)%grid%coord )
    xFld(ifld) = xtmp
    yFld(ifld) = ytmp
  ELSE
