...
! --- The "tag" derived types are used to validate the existence of certain tags,
!     and store the value extracted for those tags when possible.  This format does
!     not accommodate tags that contain multiple values.  They will be stored
!     independently in separate variables.
! --- Declare all non-character fields/arrays as 8 bytes to allow maximum flexibility.
...
! (tiff = bytes, geokey = array element)
    ...
    real    (kind=8), allocatable  :: dblVal(:)   ! real/double value
...
! --- Adjustment for unsigned integers when value is too large for target arrays
!     and is returned as negative; add this value to adjust to the real int value.
! --- Fortran does not distinguish between signed and unsigned integer data types.
