!**   PROGRAM NADCON
!**
!**   THE NATIONAL GEODETIC SURVEY NADCON PROGRAM (version 2.10)
!**   HAS BEEN ADAPTED FOR THE AERMAP TERRAIN PROCESSOR.
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: VRSION
    INTEGER :: MXAREA
!     PARAMETER (VRSION = 1.03D0, MXAREA = 8)
    PARAMETER (VRSION = 2.10D0, MXAREA = 8)

    real (kind=8) :: ADLAM, VDLAM, ADLOM, VDLOM
    real (kind=8) :: ADLAS, VDLAS, ADLOS, VDLOS
    real (kind=8) :: SDLAM, SDLAM2, SDLOM, SDLOM2
    real (kind=8) :: SDLAS, SDLAS2, SDLOS, SDLOS2
    real (kind=8) :: XSMALL, XBIG, YSMALL, YBIG
    real (kind=8) :: DLAM, DLOM, DLAS, DLOS
    real (kind=8) :: SMDLAM, BGDLAM, SMDLOM, BGDLOM
    real (kind=8) :: SMDLAS, BGDLAS, SMDLOS, BGDLOS
    real (kind=8) :: XPT, YPT, XPT2, YPT2
    INTEGER :: KEY, IPAGE, ITYPE, IFILE
    LOGICAL :: PAGE, NODATA, SCREEN,dsel

    real (kind=8) :: DX, DY, XMAX, XMIN, YMAX, YMIN
    INTEGER :: NC, NAREA
...
! wb  check for values of DLOS and DLAS out-of-range;
! wb  very large values have been seen with corrupted
! wb  grid files.  write message to main output file
    IF (DABS(DLOS) > 100.0D0 .OR. DABS(DLAS) > 100.0D0) THEN
        WRITE(LUOUT,*) ' '
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: AY, BEE, CEE, DEE
    real (kind=8) :: TEE1, TEE2, TEE3, TEE4
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: XMAX1, XMIN1, YMAX1, YMIN1
    real (kind=8) :: DX1, DY1
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: XPT, YPT, XGRID, YGRID
...
! Check to see it the point is outside the area of the gridded data
    ...
    IF (XPT >= XMAX  .OR.  XPT <= XMIN   .OR. &
    YPT >= YMAX  .OR.  YPT <= YMIN ) THEN
        NOGO = .TRUE. 
...
! Use this to change from decimal degrees (real (kind=8))
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: XPT, YPT, XPT2, YPT2, XGRID, YGRID
    real (kind=8) :: DLOS, DLAS, DLAM, DLOM
    real (kind=8) :: TEE1, TEE2, TEE3, TEE4, ZEE
    ...
    real (kind=8) :: AY1, BEE1, CEE1, DEE1, AY2, BEE2, CEE2, DEE2
...
!*********
    ...
    IF ( IROW /= IROWL  .OR.  JCOL /= JCOLL  .OR. &
    IAREA /= IAREAL ) THEN
...
!*    Since we are using negative for West longitude, reverse sign of DLOS, rwb
    DLOS = -1.0D0*ZEE
...
! I think that these are GRS80 parameters
    ...
    real (kind=8) :: AXIS, E2, RHOSEC
    PARAMETER (AXIS = 6378137.0D0)
    PARAMETER (E2 = 0.0066943800229D0)
    PARAMETER (RHOSEC = 206264.806247D0)

    real (kind=8) :: W, LM, LP, AVLAT
    real (kind=8) :: LAT1S, LAT2S, LONG1S, LONG2S, LAT1, LAT2
    real (kind=8) :: LONG1, LONG2, DLAT, DLONG
    real (kind=8) :: LATMTR, LONMTR
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: DLAM2, DLOM2, DLAS2, DLOS2
    real (kind=8) :: SDLAM, SDLAM2, SDLOM, SDLOM2
    real (kind=8) :: SDLAS, SDLAS2, SDLOS, SDLOS2
    real (kind=8) :: XSMALL, XBIG, YSMALL, YBIG, XPT, XPT2, YPT, YPT2
    real (kind=8) :: VRSION
    real (kind=8) :: SLA, SLO, SLA2, SLO2
    real (kind=8) :: DLAM, DLOM, DLAS, DLOS
    real (kind=8) :: SMDLAM, BGDLAM, SMDLOM, BGDLOM
    real (kind=8) :: SMDLAS, BGDLAS, SMDLOS, BGDLOS
...
!*        Longitude is negative West, rwb
...
! wb      SUBROUTINE NGRIDS (NODATA,dsel,VRSION)
...
! if state hpgn chosen(dsel=false) then only 1 file can be open at
! a time.  If an hpgn file is not in area.par, then the user can
! choose a state in SGRIDS.
    if(dsel) then
    ! default grids chosen
        IF (NAREA < MXAREA) THEN
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: XMAX1, XMIN1, YMAX1, YMIN1, DX, DY
...
! Form complete names of grid files
    ...
    N2 = LEN_TRIM(AFILE)
    IF (N2 == 0) STOP 'Logical Coding Error in OPENF'
...
! Check to see if the two files have the same variables
    ...
    IF ( (NC2 /= NC1)  .OR.  (NR2 /= NR1)  .OR. &
    (NZ2 /= NZ1)  .OR. &
    (X02 /= X01)  .OR.  (DX2 /= DX1)  .OR. &
    (Y02 /= Y01)  .OR.  (DY2 /= DY1)  .OR. &
    (ANGLE2 /= ANGLE1) ) GOTO 960
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: AY, BEE, CEE, DEE
    real (kind=8) :: ZEE, ZEE1, ZEE2, ZEE3, ZEE4
    INTEGER :: IROW, JCOL
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: XGRID, YGRID
    real (kind=8) :: DLAM, DLOM, DLAS, DLOS
    real (kind=8) :: DX0, DY0, XMAX0, XMIN0, YMAX0, YMIN0
...
!     IMPLICIT UNDEFINED (A-Z)
    ...
    real (kind=8) :: XPT, YPT, XPT2, YPT2
    real (kind=8) :: XTEMP, YTEMP, XDIF, YDIF
    real (kind=8) :: DLAM, DLOM, DLAS, DLOS
    real (kind=8) :: DXLAST, DYLAST
    ...
    !***************************************************************
        ...
        IF (NUM == 1) THEN
            IF (DABS(XDIF) > SMALL) THEN
                XTEMP = XPT - DLOS/3600.0D0
            ENDIF
            IF (DABS(YDIF) > SMALL) THEN
        ...
        IF (NUM >= ITMAX .OR. (DABS(YDIF) <= SMALL .AND. &
        DABS(XDIF) <= SMALL)) THEN
