...
clll. optimize
     ...
     2   ccmax, el0, h, hmin, hmxi, hu, rc, tn, uround
      real rownd2, pdest, pdlast, ratio, cm1, cm2,
     1   pdnorm
      real dcon, ddn, del, delp, dsm, dup, exdn, exsm, exup,
     1   r, rh, rhdn, rhsm, rhup, told, vmnorm
      real alpha, dm1, dm2, exm1, exm2, pdh, pnorm, rate,
...
c-----------------------------------------------------------------------
      ...
      irflag = 0
      pdh = amax1(abs(h)*pdlast,0.000001e0)
...
c-----------------------------------------------------------------------
 400  continue
      ...
      rate = amax1(rate,rm)
      crate = amax1(0.2e0*crate,rm)
 402  dcon = del*amin1(1.0e0,1.5e0*crate)/(tesco(2,nq)*conit)
      if (dcon .gt. 1.0e0) go to 405
      pdest = amax1(pdest,rate/abs(h*el(1)))
...
c if meth = 1, limit rh according to the stability region also. --------
 550  if (meth .eq. 2) go to 560
      pdh = amax1(abs(h)*pdlast,0.000001e0)
      if (l .lt. lmax) rhup = amin1(rhup,sm1(l)/pdh)
      rhsm = amin1(rhsm,sm1(nq)/pdh)
      if (nq .gt. 1) rhdn = amin1(rhdn,sm1(nq-1)/pdh)
      pdest = 0.0e0
...
c if meth = 1 and h is restricted by stability, bypass 10 percent test.
 ...
 625  if (kflag .le. -2) rh = amin1(rh,0.2e0)
c-----------------------------------------------------------------------
