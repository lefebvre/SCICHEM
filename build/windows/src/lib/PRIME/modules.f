c**********************************************************************
c   2014-Jun-26 AERMOD source code (dated 12345) modified for SCICHEM
c   as follows:
c
c   1. Add REAL variables zi_pri and wts_pri to module PRIME_AMBIENT
c******************************************************************************
      MODULE MAIN1
C***********************************************************************
C     MAIN1
...
C***********************************************************************
      ...
      INTEGER, PARAMETER :: IFMAX=150, IKN=92, ISTRG=512, ILEN_FLD=200,
     &                      IERRN=263
C*#
...
C***********************************************************************
     ...
     &   'OZONEVAL','OZONEFIL','O3VALUES','OZONUNIT','LOW_WIND',
...
C --- Newer messages, v12345:
      DATA ERRCOD(247)/'111'/,
     & ERRMSG(247)/'LOW_WIND Keyword specified without LOWWIND Options'/
      DATA ERRCOD(248)/'112'/,
     & ERRMSG(248)/'User-specified minimum Sigma-V on LOW_WIND Keyword'/
      DATA ERRCOD(249)/'113'/,
     & ERRMSG(249)/'User-specified minimum WindSpeed on LOW_WIND Keywd'/
      DATA ERRCOD(250)/'114'/,
     & ERRMSG(250)/'User-specified maximum FRAN on the LOW_WIND Keywrd'/
      DATA ERRCOD(251)/'116'/,
     & ERRMSG(251)/'Vector Wind Speeds specified on MODELOPT Keyword  '/
      DATA ERRCOD(252)/'121'/,
     & ERRMSG(252)/'LowWind1 Beta Option specified on MODELOPT Keyword'/
      DATA ERRCOD(253)/'122'/,
     & ERRMSG(253)/'LowWind2 Beta Option specified on MODELOPT Keyword'/
      DATA ERRCOD(254)/'131'/,
     & ERRMSG(254)/'Minimum sigmav value (SVmin) for LowWind1 Beta Opt'/
      DATA ERRCOD(255)/'132'/,
     & ERRMSG(255)/'Minimum sigmav value (SVmin) for LowWind2 Beta Opt'/
      DATA ERRCOD(256)/'133'/,
     & ERRMSG(256)/'Maximum FRAN value (FRANmax) for LowWind2 Beta Opt'/
      DATA ERRCOD(257)/'136'/,
     & ERRMSG(257)/'Conflicting Beta Option - LowWind1 and LowWind2   '/
      DATA ERRCOD(258)/'186'/,
     & ERRMSG(258)/'THRESH_1MIN 1-min ASOS wind speed threshold used  '/
      DATA ERRCOD(259)/'187'/,
     & ERRMSG(259)/'ADJ_U* Beta Option for Low Winds used in AERMET   '/
      DATA ERRCOD(260)/'188'/,
     & ERRMSG(260)/'Non-Default ADJ_U* Option used in AERMET without  '/
      DATA ERRCOD(261)/'341'/,
     & ERRMSG(261)/'Emissions in HOUREMIS file < -90; set to 0.0 for  '/
      DATA ERRCOD(262)/'492'/,
     & ERRMSG(262)/'SURFDATA YR .NE. 1st YR of file, adj to match file'/
      DATA ERRCOD(263)/'493'/,
     & ERRMSG(263)/'SURFDATA YR must match 1st YR of file for DAYRANGE'/
