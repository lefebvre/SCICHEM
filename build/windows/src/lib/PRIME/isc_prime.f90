...
IMPLICIT NONE
INTEGER                     :: isdx
CHARACTER*12                :: lid, hid
CHARACTER*12                :: soid
LOGICAL                     :: rmark, lfound
...
! Get The Source ID(s)
soid = TRIM(field(3))
CALL fsplit(path,keywrd,soid,12,'-',rmark,lid,hid)
! Verify The Effective Srcid
IF (lid == hid) THEN
  CALL sindex(srcid,mxsrc,soid,isdx,lfound)
  IF (.NOT.lfound) THEN
    nsrc_prime = nsrc_prime + 1
    IF( nsrc_prime > MXSRC  )THEN
      nError = WN_ERROR
      eRoutine= 'dsbldg'
      WRITE(eMessage,'("Number of prime source exceeds maximum allowed value of ",I3)')mxsrc
      eAction  = 'Stopping SCICHEM'
      GO TO 999
    END IF
    isdx = nsrc_prime
    srcid(isdx) = TRIM(ADJUSTL(soid))
  END IF
  CALL dsfill(isdx)
ELSE
  nError = WN_ERROR
...
DO WHILE (.NOT.FIND .AND. I.LE.IDIM)
   IF (TRIM(ADJUSTL(ELEM)) .EQ. TRIM(ADJUSTL(ARRIN(I)))) THEN
      FIND = .TRUE.
      INDEXS = I
   END IF
   I = I + 1
...
IMPLICIT NONE
CHARACTER             :: chk, inpfld*(*), delim, begfld*12, endfld*12, modnam*6,&
                   pathin*2, keyin*8
