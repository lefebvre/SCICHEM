...
c   as follows:
c
c   1. Replace use of ERFC with ERFC0 (original 12345 source code
c      lines 4218,4244 & 4245)
c   2. Rename function ERFC to ERFC0 and add RESULT(ERFC0)
c      (original 12345 source code line 4314)
c   3. Add calls to SCICHEM_PRIME subroutines in NUMRISE
c      - InitSCICHEM_PRIME( SNGL(fb) ) immediately before 999 CONTINUE
c         statement (12345 line 462)
c      - UpdateSCICHEM_PRIME() immediately after 999 CONTINUE statement
c      - ChkFinalSCICHEM_PRIME() in check if x .ge. xmax (after 12345
c        line 616, before GOTO 97)
c      - ChkFinalSCICHEM_PRIME() after incrementing NNP (12345 line 630)
c   4. In subroutine ZMET, set zero potential temperature gradient if
c      inside convective boundary layer, i.e.,
c      if (wts_pri >= 0. .and. z <= zi_pri) dpdz = 0.
c********************************************************************** 
c----------------------------------------------------------------------
      subroutine prime1
c----------------------------------------------------------------------
...
c --- START MARCHING LOOP
      ...
      call LUMP(ua,ta,f)
c************************************
      CALL InitSCICHEM_PRIME( SNGL(fb) )
C************************************

999   continue

c************************************
      CALL UpdateSCICHEM_PRIME()
C************************************
...
C --- Save original SVATZ before applying minimum value, for use
...
CRWB  write-up by Steve Perry.  R. Brode, PES, 8/15/96
      ...
      IF( L_VECTORWS )THEN
         IF (L_LowWind1 .OR. L_LowWind2) THEN
            UA = DSQRT( UATZ*UATZ + 2.0D0*SVATZ2*SVATZ2 )
            UA = MAX( UATZ, WSMIN )
         ELSE
...
c******************************************************************************
c --- set potential temperature gradient to zero inside convective BL
      if (wts_pri >= 0. .and. z <= zi_pri) dpdz = 0.
c******************************************************************************
...
c     IF(XX2.LT.13.306D0) ERFCX2=ERFC(XX2)
      if(xx1 .LT. 9.15D0) erfcx1=ERFC0(xx1)
      if(xx2 .LT. 9.15D0) erfcx2=ERFC0(xx2)
...
c-----------------------------------------------------------------------
      DOUBLE PRECISION FUNCTION ERFC0(XX)  RESULT(ERFC)
c-----------------------------------------------------------------------
c
