...
* Fullsource for module SVODE from package ODE.
* Retrieved from NETLIB on Tue Jun 16 20:57:19 1998.
* ======================================================================

*DECK SVODE
      SUBROUTINE SVODE_(F, NEQ, Y, T, TOUT, ITOL, RTOL, ATOL, ITASK,
...
C     SUBROUTINE SEWSET_(NEQ, ITOL, RTOL, ATOL, YCUR, EWT)
...
C MAXORD was reduced below NQ.  Copy YH(*,MAXORD+2) into SAVF. ---------
      CALL SCOPY_(N, RWORK(LWM), 1, RWORK(LSAVF), 1)
...
C Load the initial value vector in YH. ---------------------------------
      CALL SCOPY_(N, Y, 1, RWORK(LYH), 1)
C Load and invert the EWT array.  (H is temporarily set to 1.0.) -------
      NQ = 1
      H = ONE
      CALL SEWSET_(N, ITOL, RTOL, ATOL, RWORK(LYH), RWORK(LEWT))
      DO 120 I = 1,N
...
C Call SVHIN to set initial step size H0 to be attempted. --------------
      CALL SVHIN_(N, T, RWORK(LYH), RWORK(LF0), F, RPAR, IPAR, TOUT,
...
C-----------------------------------------------------------------------
 ...
 210  IF ((TN - TOUT)*H .LT. ZERO) GO TO 250
      CALL SVINDY_(TOUT, 0, RWORK(LYH), NYH, Y, IFLAG)
...
C-----------------------------------------------------------------------
 ...
 270  TOLSF = UROUND*SVNORM_(N, RWORK(LYH), RWORK(LEWT))
 ...
 280  IF ((TN + H) .NE. TN) GO TO 290
      ...
      CALL XERRWV_(MSG, 50, 101, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG='      such that in the machine, T + H = T on the next step  '
      CALL XERRWV_(MSG, 60, 101, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      (H = step size). solver will continue anyway'
      CALL XERRWV_(MSG, 50, 101, 1, 0, 0, 0, 2, TN, H)
      IF (NHNIL .LT. MXHNIL) GO TO 290
      MSG = 'SVODE--  Above warning has been issued I1 times.  '
      CALL XERRWV_(MSG, 50, 102, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      it will not be issued again for this problem'
      CALL XERRWV_(MSG, 50, 102, 1, 1, MXHNIL, 0, 0, ZERO, ZERO)
 290  CONTINUE
C-----------------------------------------------------------------------
C CALL SVSTEP (Y, YH, NYH, YH, EWT, SAVF, VSAV, ACOR,
C              WM, IWM, F, JAC, F, SVNLSD, RPAR, IPAR)
C-----------------------------------------------------------------------
      CALL SVSTEP_(Y, RWORK(LYH), NYH, RWORK(LYH), RWORK(LEWT),
     1   RWORK(LSAVF), Y, RWORK(LACOR), RWORK(LWM), IWORK(LIWM),
     2   F, JAC, F, SVNLSD_, RPAR, IPAR)
      KGO = 1 - KFLAG
...
C The maximum number of steps was taken before reaching TOUT. ----------
 500  MSG = 'SVODE--  At current T (=R1), MXSTEP (=I1) steps   '
      CALL XERRWV_(MSG, 50, 201, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      taken on this call before reaching TOUT     '
      CALL XERRWV_(MSG, 50, 201, 1, 1, MXSTEP, 0, 1, TN, ZERO)
      ISTATE = -1
      GO TO 580
C EWT(i) .le. 0.0 for some i (not at start of problem). ----------------
 510  EWTI = RWORK(LEWT+I-1)
      MSG = 'SVODE--  At T (=R1), EWT(I1) has become R2 .le. 0.'
      CALL XERRWV_(MSG, 50, 202, 1, 1, I, 0, 2, TN, EWTI)
      ISTATE = -6
      GO TO 580
C Too much accuracy requested for machine precision. -------------------
 520  MSG = 'SVODE--  At T (=R1), too much accuracy requested  '
      CALL XERRWV_(MSG, 50, 203, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      for precision of machine..  see TOLSF (=R2) '
      CALL XERRWV_(MSG, 50, 203, 1, 0, 0, 0, 2, TN, TOLSF)
      RWORK(14) = TOLSF
      ISTATE = -2
      GO TO 580
C KFLAG = -1.  Error test failed repeatedly or with ABS(H) = HMIN. -----
 530  MSG = 'SVODE--  At T(=R1) and step size H(=R2), the error'
      CALL XERRWV_(MSG, 50, 204, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      test failed repeatedly or with abs(H) = HMIN'
      CALL XERRWV_(MSG, 50, 204, 1, 0, 0, 0, 2, TN, H)
      ISTATE = -4
      GO TO 560
C KFLAG = -2.  Convergence failed repeatedly or with ABS(H) = HMIN. ----
 540  MSG = 'SVODE--  At T (=R1) and step size H (=R2), the    '
      CALL XERRWV_(MSG, 50, 205, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      corrector convergence failed repeatedly     '
      CALL XERRWV_(MSG, 50, 205, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG = '      or with abs(H) = HMIN   '
      CALL XERRWV_(MSG, 30, 205, 1, 0, 0, 0, 2, TN, H)
      ISTATE = -5
...
C-----------------------------------------------------------------------
 601  MSG = 'SVODE--  ISTATE (=I1) illegal '
      CALL XERRWV_(MSG, 30, 1, 1, 1, ISTATE, 0, 0, ZERO, ZERO)
      IF (ISTATE .LT. 0) GO TO 800
      GO TO 700
 602  MSG = 'SVODE--  ITASK (=I1) illegal  '
      CALL XERRWV_(MSG, 30, 2, 1, 1, ITASK, 0, 0, ZERO, ZERO)
      GO TO 700
 603  MSG='SVODE--  ISTATE (=I1) .gt. 1 but SVODE not initialized      '
      CALL XERRWV_(MSG, 60, 3, 1, 1, ISTATE, 0, 0, ZERO, ZERO)
      GO TO 700
 604  MSG = 'SVODE--  NEQ (=I1) .lt. 1     '
      CALL XERRWV_(MSG, 30, 4, 1, 1, NEQ, 0, 0, ZERO, ZERO)
      GO TO 700
 605  MSG = 'SVODE--  ISTATE = 3 and NEQ increased (I1 to I2)  '
      CALL XERRWV_(MSG, 50, 5, 1, 2, N, NEQ, 0, ZERO, ZERO)
      GO TO 700
 606  MSG = 'SVODE--  ITOL (=I1) illegal   '
      CALL XERRWV_(MSG, 30, 6, 1, 1, ITOL, 0, 0, ZERO, ZERO)
      GO TO 700
 607  MSG = 'SVODE--  IOPT (=I1) illegal   '
      CALL XERRWV_(MSG, 30, 7, 1, 1, IOPT, 0, 0, ZERO, ZERO)
      GO TO 700
 608  MSG = 'SVODE--  MF (=I1) illegal     '
      CALL XERRWV_(MSG, 30, 8, 1, 1, MF, 0, 0, ZERO, ZERO)
      GO TO 700
 609  MSG = 'SVODE--  ML (=I1) illegal.. .lt.0 or .ge.NEQ (=I2)'
      CALL XERRWV_(MSG, 50, 9, 1, 2, ML, NEQ, 0, ZERO, ZERO)
      GO TO 700
 610  MSG = 'SVODE--  MU (=I1) illegal.. .lt.0 or .ge.NEQ (=I2)'
      CALL XERRWV_(MSG, 50, 10, 1, 2, MU, NEQ, 0, ZERO, ZERO)
      GO TO 700
 611  MSG = 'SVODE--  MAXORD (=I1) .lt. 0  '
      CALL XERRWV_(MSG, 30, 11, 1, 1, MAXORD, 0, 0, ZERO, ZERO)
      GO TO 700
 612  MSG = 'SVODE--  MXSTEP (=I1) .lt. 0  '
      CALL XERRWV_(MSG, 30, 12, 1, 1, MXSTEP, 0, 0, ZERO, ZERO)
      GO TO 700
 613  MSG = 'SVODE--  MXHNIL (=I1) .lt. 0  '
      CALL XERRWV_(MSG, 30, 13, 1, 1, MXHNIL, 0, 0, ZERO, ZERO)
      GO TO 700
 614  MSG = 'SVODE--  TOUT (=R1) behind T (=R2)      '
      CALL XERRWV_(MSG, 40, 14, 1, 0, 0, 0, 2, TOUT, T)
      MSG = '      integration direction is given by H0 (=R1)  '
      CALL XERRWV_(MSG, 50, 14, 1, 0, 0, 0, 1, H0, ZERO)
      GO TO 700
 615  MSG = 'SVODE--  HMAX (=R1) .lt. 0.0  '
      CALL XERRWV_(MSG, 30, 15, 1, 0, 0, 0, 1, HMAX, ZERO)
      GO TO 700
 616  MSG = 'SVODE--  HMIN (=R1) .lt. 0.0  '
      CALL XERRWV_(MSG, 30, 16, 1, 0, 0, 0, 1, HMIN, ZERO)
      GO TO 700
 617  CONTINUE
      MSG='SVODE--  RWORK length needed, LENRW (=I1), exceeds LRW (=I2)'
      CALL XERRWV_(MSG, 60, 17, 1, 2, LENRW, LRW, 0, ZERO, ZERO)
      GO TO 700
 618  CONTINUE
      MSG='SVODE--  IWORK length needed, LENIW (=I1), exceeds LIW (=I2)'
      CALL XERRWV_(MSG, 60, 18, 1, 2, LENIW, LIW, 0, ZERO, ZERO)
      GO TO 700
 619  MSG = 'SVODE--  RTOL(I1) is R1 .lt. 0.0        '
      CALL XERRWV_(MSG, 40, 19, 1, 1, I, 0, 1, RTOLI, ZERO)
      GO TO 700
 620  MSG = 'SVODE--  ATOL(I1) is R1 .lt. 0.0        '
      CALL XERRWV_(MSG, 40, 20, 1, 1, I, 0, 1, ATOLI, ZERO)
      GO TO 700
 621  EWTI = RWORK(LEWT+I-1)
      MSG = 'SVODE--  EWT(I1) is R1 .le. 0.0         '
      CALL XERRWV_(MSG, 40, 21, 1, 1, I, 0, 1, EWTI, ZERO)
      GO TO 700
 622  CONTINUE
      MSG='SVODE--  TOUT (=R1) too close to T(=R2) to start integration'
      CALL XERRWV_(MSG, 60, 22, 1, 0, 0, 0, 2, TOUT, T)
      GO TO 700
 623  CONTINUE
      MSG='SVODE--  ITASK = I1 and TOUT (=R1) behind TCUR - HU (= R2)  '
      CALL XERRWV_(MSG, 60, 23, 1, 1, ITASK, 0, 2, TOUT, TP)
      GO TO 700
 624  CONTINUE
      MSG='SVODE--  ITASK = 4 or 5 and TCRIT (=R1) behind TCUR (=R2)   '
      CALL XERRWV_(MSG, 60, 24, 1, 0, 0, 0, 2, TCRIT, TN)
      GO TO 700
 625  CONTINUE
      MSG='SVODE--  ITASK = 4 or 5 and TCRIT (=R1) behind TOUT (=R2)   '
      CALL XERRWV_(MSG, 60, 25, 1, 0, 0, 0, 2, TCRIT, TOUT)
      GO TO 700
 626  MSG = 'SVODE--  At start of problem, too much accuracy   '
      CALL XERRWV_(MSG, 50, 26, 1, 0, 0, 0, 0, ZERO, ZERO)
      MSG='      requested for precision of machine..  see TOLSF (=R1) '
      CALL XERRWV_(MSG, 60, 26, 1, 0, 0, 0, 1, TOLSF, ZERO)
      RWORK(14) = TOLSF
      GO TO 700
 627  MSG='SVODE--  Trouble from SVINDY.  ITASK = I1, TOUT = R1.       '
      CALL XERRWV_(MSG, 60, 27, 1, 1, ITASK, 0, 1, TOUT, ZERO)
C
 700  CONTINUE
      ISTATE = -3
      RETURN
C
 800  MSG = 'SVODE--  Run aborted.. apparent infinite loop     '
      CALL XERRWV_(MSG, 50, 303, 2, 0, 0, 0, 0, ZERO, ZERO)
      RETURN
C----------------------- End of Subroutine SVODE -----------------------
      END
*DECK SVHIN
      SUBROUTINE SVHIN_(N, T0, Y0, YDOT, F, RPAR, IPAR, TOUT, UROUND,
...
C----------------------- End of Subroutine SVHIN -----------------------
      END
*DECK SVINDY
      SUBROUTINE SVINDY_(T, K, YH, LDYH, DKY, IFLAG)
      REAL T, YH, DKY
...
C
 ...
 50     CONTINUE
      IF (K .EQ. 0) RETURN
 55   R = H**(-K)
      CALL SSCAL_(N, R, DKY, 1)
      RETURN
C
 80   MSG = 'SVINDY-- K (=I1) illegal      '
      CALL XERRWV_(MSG, 30, 51, 1, 1, K, 0, 0, ZERO, ZERO)
      IFLAG = -1
      RETURN
 90   MSG = 'SVINDY-- T (=R1) illegal      '
      CALL XERRWV_(MSG, 30, 52, 1, 0, 0, 0, 1, T, ZERO)
      MSG='      T not in interval TCUR - HU (= R1) to TCUR (=R2)      '
      CALL XERRWV_(MSG, 60, 52, 1, 0, 0, 0, 2, TP, TN)
      IFLAG = -2
      RETURN
C----------------------- End of Subroutine SVINDY ----------------------
      END
*DECK SVSTEP
      SUBROUTINE SVSTEP_(Y, YH, LDYH, YH1, EWT, SAVF, VSAV, ACOR,
...
C-----------------------------------------------------------------------
 ...
 470    TAU(I+1) = TAU(I)
      TAU(1) = H
      DO 480 J = 1, L
        CALL SAXPY_(N, EL(J), ACOR, 1, YH(1,J), 1 )
 480    CONTINUE
      NQWAIT = NQWAIT - 1
      IF ((L .EQ. LMAX) .OR. (NQWAIT .NE. 1)) GO TO 490
      CALL SCOPY_(N, ACOR, 1, YH(1,LMAX), 1 )
      CONP = TQ(5)
...
C----------------------- End of Subroutine SVSET -----------------------
      END
*DECK SVJUST
      SUBROUTINE SVJUST_(YH, LDYH, IORD)
      REAL YH
      INTEGER LDYH, IORD
      DIMENSION YH(LDYH,*)
C-----------------------------------------------------------------------
...
C Add correction terms to YH array. ------------------------------------
      NQP1 = NQ + 1
      DO 370 J = 3, NQP1
        CALL SAXPY_(N, EL(J), YH(1,LP1), 1, YH(1,J), 1 )
 370  CONTINUE
      RETURN
C----------------------- End of Subroutine SVJUST ----------------------
      END
*DECK SVNLSD
      SUBROUTINE SVNLSD_(Y, YH, LDYH, VSAV, SAVF, EWT, ACOR, IWM, WM,
...
C-----------------------------------------------------------------------
      CALL SVJAC_(Y, YH, LDYH, EWT, ACOR, SAVF, WM, IWM, F, JAC, IERPJ,
     1           RPAR, IPAR)
      IPUP = 0
      RC = ONE
      DRC = ZERO
...
C-----------------------------------------------------------------------
 ...
 360    Y(I) = (RL1*H)*SAVF(I) - (RL1*YH(I,2) + ACOR(I))
      CALL SVSOL_(WM, IWM, Y, IERSL)
      NNI = NNI + 1
      IF (IERSL .GT. 0) GO TO 410
      IF (METH .EQ. 2 .AND. RC .NE. ONE) THEN
        CSCALE = TWO/(ONE + RC)
        CALL SSCAL_(N, CSCALE, Y, 1)
      ENDIF
      DEL = SVNORM_(N, Y, EWT)
      CALL SAXPY_(N, ONE, Y, 1, ACOR, 1)
      DO 380 I = 1,N
...
C Return for successful step. ------------------------------------------
 450  NFLAG = 0
      ...
      ICF = 0
      IF (M .EQ. 0) ACNRM = DEL
      IF (M .GT. 0) ACNRM = SVNORM_(N, ACOR, EWT)
      RETURN
C----------------------- End of Subroutine SVNLSD ----------------------
      END
*DECK SVJAC
      SUBROUTINE SVJAC_(Y, YH, LDYH, EWT, FTEM, SAVF, WM, IWM, F, JAC,
     1                 IERPJ, RPAR, IPAR)
      EXTERNAL F, JAC
...
C If JOK = -1 and MITER = 1, call JAC to evaluate Jacobian. ------------
 ...
 110    WM(I+2) = ZERO
      ...
      IF (JSV .EQ. 1) CALL SCOPY_(LENP, WM(3), 1, WM(LOCJS), 1)
      ENDIF
C
...
C Multiply Jacobian by scalar, add identity, and do LU decomposition. --
 ...
 250    J = J + NP1
      NLU = NLU + 1
      CALL SGEFA_(WM(3), N, N, IWM(31), IER)
      IF (IER .NE. 0) IERPJ = 1
      RETURN
      ENDIF
...
C If JOK = -1 and MITER = 4, call JAC to evaluate Jacobian. ------------
 ...
 410    WM(I+2) = ZERO
      ...
      IF (JSV .EQ. 1)
     1   CALL SACOPY_(MBAND, N, WM(ML3), MEBAND, WM(LOCJS), MBAND)
      ENDIF
C
...
C
      IF (JOK .EQ. 1) THEN
      JCUR = 0
      CALL SACOPY_(MBAND, N, WM(LOCJS), MBAND, WM(ML3), MEBAND)
      ENDIF
C
C Multiply Jacobian by scalar, add identity, and do LU decomposition.
      CON = -HRL1
 ...
 580    II = II + MEBAND
      NLU = NLU + 1
      CALL SGBFA_(WM(3), MEBAND, N, ML, MU, IWM(31), IER)
      IF (IER .NE. 0) IERPJ = 1
      RETURN
...
C----------------------- End of Subroutine SVJAC -----------------------
      END
*DECK SACOPY
      SUBROUTINE SACOPY_(NROW, NCOL, A, NROWA, B, NROWB)
      REAL A, B
...
C
      DO 20 IC = 1,NCOL
        CALL SCOPY_(NROW, A(1,IC), 1, B(1,IC), 1)
 20     CONTINUE
C
      RETURN
C----------------------- End of Subroutine SACOPY ----------------------
      END
*DECK SVSOL
      SUBROUTINE SVSOL_(WM, IWM, X, IERSL)
      REAL WM, X
      INTEGER IWM, IERSL
      DIMENSION WM(*), IWM(*), X(*)
C-----------------------------------------------------------------------
...
C
 ...
 100  CALL SGESL_(WM(3), N, N, IWM(31), X, 0)
      RETURN
C
 300  PHRL1 = WM(2)
      HRL1 = H*RL1
...
C
 400  ML = IWM(1)
      MU = IWM(2)
      MEBAND = 2*ML + MU + 1
      CALL SGBSL_(WM(3), MEBAND, N, ML, MU, IWM(31), X, 0)
      RETURN
C----------------------- End of Subroutine SVSOL -----------------------
      END
*DECK SVSRCO
      SUBROUTINE SVSRCO_(RSAV, ISAV, JOB)
      REAL RSAV
      INTEGER ISAV, JOB
      DIMENSION RSAV(*), ISAV(*)
C-----------------------------------------------------------------------
...
C----------------------- End of Subroutine SVSRCO ----------------------
      END
*DECK SEWSET
      SUBROUTINE SEWSET_(N, ITOL, RTOL, ATOL, YCUR, EWT)
...
C
 ...
 10     SUM = SUM + (V(I)*W(I))**2
      SVNORM_ = SQRT(SUM/REAL(N))
      RETURN
C----------------------- End of Function SVNORM ------------------------
      END
*DECK R1MACH
      REAL FUNCTION R1MACH_(IDUM)
      INTEGER IDUM
C-----------------------------------------------------------------------
...
C
C Subroutines/functions called by R1MACH.. None
C-----------------------------------------------------------------------
      REAL U, COMP
      U = 1.0E0
 10   U = U*0.5E0
...
C----------------------- End of Function R1MACH ------------------------
      END
*DECK XERRWV
      SUBROUTINE XERRWV_(MSG, NMES, NERR, LEVEL, NI, I1, I2, NR, R1, R2)
      REAL R1, R2
...
C----------------------- End of Function IXSAV -------------------------
      END
c ROUTINES SELECTED FROM BLAS AND USED BY VODE

      subroutine SCOPY_(n,sx,incx,sy,incy)
c
...
c
   ...
   50 continue
      ...
      subroutine SSCAL_(n,sa,sx,incx)
c
...
c
   ...
   50 continue
      ...
      subroutine SAXPY_(n,sa,sx,incx,sy,incy)
c
...
c LINPACK ROUTINES USED BY SVODE


* ======================================================================
* NIST Guide to Available Math Software.
* Fullsource for module SGEFA from package LINPACK.
* Retrieved from NETLIB on Wed Jun 17 08:00:19 1998.
* ======================================================================
      subroutine SGEFA_(a,lda,n,ipvt,info)
...
c
            do 30 j = kp1, n
               ...
               if (l .eq. k) go to 20
                  ...
                  a(k,j) = t
   20          continue
               call SAXPY_(n-k,t,a(k+1,k),1,a(k+1,j),1)
   30       continue
         go to 50
   40    continue
            info = k
   ...
   70 continue
      ...
      integer function isamax_(n,sx,incx)
c
...
* ======================================================================
* NIST Guide to Available Math Software.
* Fullsource for module SGBFA from package LINPACK.
* Retrieved from NETLIB on Wed Jun 17 08:01:13 1998.
* ======================================================================
      subroutine SGBFA_(abd,lda,n,ml,mu,ipvt,info)
...
c
      real t
      integer i,isamax_,i0,j,ju,jz,j0,j1,k,kp1,l,lm,m,mm,nm1
c
c
      m = ml + mu + 1
      info = 0
...
c
            ...
            do 80 j = kp1, ju
               ...
               if (l .eq. mm) go to 70
                  ...
                  abd(mm,j) = t
   70          continue
               call SAXPY_(lm,t,abd(m+1,k),1,abd(mm+1,j),1)
   80       continue
   90       continue
         go to 110
  100    continue
...
* ======================================================================
* NIST Guide to Available Math Software.
* Fullsource for module SGESL from package LINPACK.
* Retrieved from NETLIB on Wed Jun 17 08:01:47 1998.
* ======================================================================
      subroutine SGESL_(a,lda,n,ipvt,b,job)
...
c
         ...
         do 20 k = 1, nm1
            ...
            if (l .eq. k) go to 10
               ...
               b(k) = t
   10       continue
            call SAXPY_(n-k,t,a(k+1,k),1,b(k+1),1)
   20    continue
   30    continue
c
c        now solve  u*x = y
c
         do 40 kb = 1, n
            ...
            b(k) = b(k)/a(k,k)
            t = -b(k)
            call SAXPY_(k-1,t,a(1,k),1,b(1),1)
   40    continue
      go to 100
   50 continue
c
...
c
         ...
         do 80 kb = 1, nm1
            k = n - kb
            b(k) = b(k) + sdot_(n-k,a(k+1,k),1,b(k+1),1)
            l = ipvt(k)
            if (l .eq. k) go to 70
               t = b(l)
               b(l) = b(k)
...
c
   ...
   60 sdot_ = stemp
      ...
      subroutine SGBSL_(abd,lda,n,ml,mu,ipvt,b,job)
         ...
         if (nm1 .lt. 1) go to 30
            do 20 k = 1, nm1
               ...
               if (l .eq. k) go to 10
                  ...
                  b(k) = t
   10          continue
               call SAXPY_(lm,t,abd(m+1,k),1,b(k+1),1)
   20       continue
   30    continue
c
c        now solve  u*x = y
c
         do 40 kb = 1, n
            ...
            lb = k - lm
            t = -b(k)
            call SAXPY_(lm,t,abd(la,k),1,b(lb),1)
   40    continue
      go to 100
   50 continue
c
...
c
         ...
         if (nm1 .lt. 1) go to 90
            do 80 kb = 1, nm1
               k = n - kb
               lm = min0(ml,n-k)
               b(k) = b(k) + sdot_(lm,abd(m+1,k),1,b(k+1),1)
               l = ipvt(k)
               if (l .eq. k) go to 70
                  t = b(l)
                  b(l) = b(k)
