...
c         /lsa001/  of length  31    (22 single precision words
c                         followed by  9 integer words),
c         /lsr001/  of length  14     (5 single precision words
c                         followed by  9 integer words),
...
c
c note.. in this version of lsodar, there are two data statements,
c in subroutines lsodar and xerrwv, which load variables into these
...
c           linear systems.
c  saxpy, sscal, isamax, sdot, and scopy   are basic linear algebra
...
c note..  vmnorm, fnorm, bnorm, isamax, sdot, and r1mach are function
...
c-----------------------------------------------------------------------
      call load_lsode_bd                                                !sfp - block data
...
c if istate = 3 and ng was changed, shift yh to its new location. ------
 ...
 65   lsavf = lewt + n
     ...
     1   60, 104, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrwv(
     1  60h      may not be later.  integration will proceed anyway.   ,
     1   60, 104, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrwv(
     1  50h      length needed is leniw = i1, while liw = i2.,
     1   50, 104, 0, 2, leniw, liw, 0, 0.0e0, 0.0e0)
 70   continue
...
c rwork length too small to proceed. -----------------------------------
 550  call xerrwv(50hlsodar-  at current t(=r1), rwork length too small,
     1   50, 206, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrwv(
     1  60h      to proceed.  the integration was otherwise successful.,
     1   60, 206, 0, 0, 0, 0, 1, tn, 0.0e0)
      istate = -7
      go to 580
...
c-----------------------------------------------------------------------
 ...
 611  call xerrwv(30hlsodar-  ixpr (=i1) illegal   ,
     1   30, 11, 0, 1, ixpr, 0, 0, 0.0e0, 0.0e0)
      go to 700
