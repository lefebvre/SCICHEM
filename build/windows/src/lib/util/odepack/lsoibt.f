...
c
c note.. in this version of lsoibt, there are two data statements,
c in subroutines lsoibt and xerrwv, which load variables into these
...
c
c the intrinsic and external routines used by lsoibt are.. abs,
c amax1, amin1, float, iabs, max0, min0, mod, sign, sqrt, and write.
c
c-----------------------------------------------------------------------
...
c kflag = -1.  error test failed repeatedly or with abs(h) = hmin. -----
 530  call xerrwv(50hlsoibt-- at t (=r1) and step size h (=r2), the    ,
     1   50, 204, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrwv(50herror test failed repeatedly or with abs(h) = hmin,
...
c compute residual if relevant. ----------------------------------------
 ...
 585     y(i) = rwork(i+lyh-1)
     ...
     1   50, 210, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrwv(50h      to (i1) when called for final output.       ,
     1   50, 210, 0, 1, ires, 0, 0, 0.0e0, 0.0e0)
      go to 595
...
c-----------------------------------------------------------------------
 ...
 609  call xerrwv(40hlsoibt-- mb (=i1) or nb (=i2) illegal   ,
     1   50, 9, 0, 2, mb, nb, 0, 0.0e0, 0.0e0)
      go to 700
 610  call xerrwv(40hlsoibt-- nb(=i1) illegal.. .lt. 4       ,
     1   50, 10, 0, 1, nb, 0, 0, 0.0e0, 0.0e0)
      go to 700
