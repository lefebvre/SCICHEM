      subroutine lsode_sl (f, neq, y, t, tout, itol, rtol, atol, itask,
     1            istate, iopt, rwork, lrw, iwork, liw, jac, mf)
      external f, jac
      integer neq, itol, itask, istate, iopt, lrw, iwork, liw, mf
      real y, t, tout, rtol, atol, rwork, tlast
...
cgy
c this version calls subroutine slow_step after each successful step
c by the core integrator stode.  in scichem, slow_step is used to
c step the concentrations of slow species, if present.
c for now, slow_step is called only if itask = 1 because the logic
c for other values of itask may be different
cgy
c
...
c for other values of itask may be different
c
      if (itask .eq. 1) call slow_step (tn, tout, tlast, y)
cgy
