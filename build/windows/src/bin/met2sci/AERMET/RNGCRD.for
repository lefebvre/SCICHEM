      SUBROUTINE RNGCRD(KOUNT,CARD,NAME,KEY,MISS,LBOUND,UBOUND,ISTAT)
C=====================================================================**
C          RNGCRD Module of the AERMET Meterological Preprocessor
C
C     Purpose: Reads a RANGE keyword record for the UA and SF pathways
C              that the user has specified to change the QA value for 
C              one or more of the parameters for the variable
C
C     Called By: UACARD, SFCARD
C
...
C
C     NOTE:  user must specify lower, upper, and missing as type INTEGER
C
C-----------------------------------------------------------------------
...
3000           FORMAT(' ERROR DECODING MISSING INDICATOR FOR: ',A8)
               CALL ERRHDL( KOUNT,PATH,ECODE,LOC,MESS )
               ISTAT = 1
            ENDIF
...
4000           FORMAT(' ERROR DECODING THE LOWER BOUND FOR: ',A8)
               CALL ERRHDL( KOUNT,PATH,ECODE,LOC,MESS )
               ISTAT = 1
            ENDIF
         ELSE
...
5000           FORMAT(' ERROR DECODING THE UPPER BOUND FOR: ',A8)
               CALL ERRHDL( KOUNT,PATH,ECODE,LOC,MESS )
               ISTAT = 1
            ENDIF
         ELSE
