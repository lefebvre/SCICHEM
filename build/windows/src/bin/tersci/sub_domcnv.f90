SUBROUTINE DOMCNV
  !***********************************************************************
  !*                DOMCNV Module of AERMAP Terrain Preprocessor
  !*
  !*       PURPOSE: To Synchronize the Coordinate Systems of the Domain
  !*                and the Terrain File Type.  If the Terrain File is of
  !*                Type 'LAT', then the Domain Extents Should be in Lat/Lon
  !*                Else if is of Type 'UTM', then the Domain Extents Should
  !*                be in UTM.
  !*
  ...
  !*
  !*       INPUTS: Coordinates of the Corners of the Domain
  !*
  !*       OUTPUTS:Coordinates of the Corners of the Domain
  !*               in a System Consistent With the Terrain Data
  !*
  !*       CALLED FROM:   MAIN
  !***********************************************************************
  ...
  !*    Use the datum from the ANCHORXY input line (NADA) to define the domain.
  !*    Default is GRS80 for 1-deg (LAT) and Clarke 1866 for 7.5min (UTM) files.
...
!**** Process Domain Extents based on TYPDEM and DOMTYP.  Ultimately, the
!*    Domain will be interpreted based on TYPDEM so that domain extents will
!*    follow profiles of DEM data files.  Therefore, for 1-degree DEM and other
!*    geographic elevation data (TYPDEM = 'LAT') the domain will be interpreted
!*    in terms of Lat/Lon regardless of whether DOMAINLL or DOMAINXY is used.
!*    Likewise, for 7.5-minute DEM and other UTM elevation data (TYPDEM = 'UTM')
!**** the domain will be interpreted in terms of UTM-X, UTM-Y and UTM-Zone.

  IF (TYPDEM == 'LAT') THEN
! ---    DEM/NED data based on geographic coordinates

     IF (DOMTYP == 'LAT') THEN
!*          Check the latitude and issue a warning if it exceeds 66 deg -
!*          reason: latitude and longitude may be switched on DOMAINLL card;

        IF( DABS(YDMIN) > 66.0D0 )THEN
           WRITE (DUMMY,'(f8.2)' ) YDMIN
           CALL ERRHDL(PATH,MODNAM,'W','360',DUMMY)
        END IF

        IF( DABS(YDMAX) > 66.0D0 )THEN
           WRITE (DUMMY,'(f8.2)' ) YDMAX
           CALL ERRHDL(PATH,MODNAM,'W','360',DUMMY)
        END IF

!*          Convert domain extents to arc-seconds and
!*          save domain extents in DOMLL array.  First index is 1 for Lat,
!*          2 for Lon; second index is 1 to 4 clockwise from SW corner
!*
     ...
     ELSE IF (DOMTYP == 'UTM') THEN
!*          Convert the Extents from UTM to LAT/LON, using ISPHERE based on NADA
        XARG  = XDMIN
        YARG  = YDMIN
        IZONE = ZONMIN
        ARGE  = 0.0D0
...
!*       Assign SW and NE DOMLL values to other corners
     DOMLL(2,2) = DOMLL(2,1)     ! NW corner
     DOMLL(1,2) = DOMLL(1,3)     ! NW corner
     DOMLL(2,4) = DOMLL(2,3)     ! SE corner
     DOMLL(1,4) = DOMLL(1,1)     ! SE corner

!*       Determine UTM coordinates for SW and NE corners and store
!*       in XDMIN/XDMAX and YDMIN/YDMAX variables
     ARGE   = DOMLL(2,1)
     ARGN   = DOMLL(1,1)
     XARG   = 0.0D0
     YARG   = 0.0D0
...
!*       Determine NAD shift for Domain:
...
!*
!*          First calculate shift for SW Corner (XDMIN, YDMIN)
!*
...
!*          Don't need UTM Domain Shifts for geographic elevation files
        XDMNDIFM = 0.0D0
        YDMNDIFM = 0.0D0

!*
!*          Next calculate shift for NW Corner (XDMAX, YDMAX)
!*
...
! ---    DEM/NED data based on UTM coordinates

     IF (DOMTYP == 'LAT') THEN
!*          Check the latitude and issue a warning if it exceeds 66 deg -
!*          reason: latitude and longitude may be switched on DOMAINLL card;
!*          convert latitude and longitude to arc-seconds and then to UTM
...
!*          Calculate the Coords in UTM for SW and NE corners based on
!*            Lat/Lon, relative to NADA ellipsoid;
!*            first convert to arc-seconds
        ARGE   =  XDMIN * 3600.0D0
        ARGN   =  YDMIN * 3600.0D0
        XARG   = 0.0D0
        YARG   = 0.0D0
...
!*          Convert SW corner UTMs to Lat/Long relative to NADA ellipsoid
        XARG  = XDMIN
        YARG  = YDMIN
        IZONE = ZONMIN
        ARGE  = 0.0D0
...
!*          Convert back to UTM to check for zone change
        ...
        IF (IZNEW /= IZONE) THEN
           DOMLL(2,1) = ARGE
           DOMLL(1,1) = ARGN
!*             Save new UTM coordinates and zone of domain SW corner
           XDMIN  = XARG
           YDMIN  = YARG
           ZONMIN = IZNEW
        END IF

!*          Convert NE corner UTMs to Lat/Long relative to NADA ellipsoid
        XARG  = XDMAX
        YARG  = YDMAX
        IZONE = ZONMAX
        ARGE  = 0.0D0
...
!*          Convert back to UTM to check for zone change
        ...
        IF (IZNEW /= IZONE) THEN
           DOMLL(2,3) = ARGE
           DOMLL(1,3) = ARGN
!*             Save new UTM coordinates and zone of domain NE corner
           XDMAX  = XARG
           YDMAX  = YARG
           ZONMAX = IZNEW
        END IF
...
!*       Make final conversion to DOMLL array
     XARG = XDMIN
     YARG = YDMIN
     IZONE = ZONMIN
     ARGE  = 0.0D0
...
!*       Determine Lat/Lon of NW and SE corners based on UTMs from DOMAINXY
!*       Make final conversion to DOMLL array
!*       NW Corner:
     XARG = XDMIN
     YARG = YDMAX
...
!*          Now calculate full UTM shift including ellipsoid and datum shifts
        ...
        ZONMIN_SHFT = IZONE
!*
!*          Next calculate shift for NE Corner (XDMAX, YDMAX)
!*
        XPT27 = DOMLL(2,3)/3600.0D0
        YPT27 = DOMLL(1,3)/3600.0D0
...
!*    Open DOMDETAIL.OUT debug file
  OPEN(UNIT=DOMK, FILE=DOMDET_FILE, STATUS='REPLACE')
!*    Write Version Date Header to DOMDETAIL Debug File
  WRITE(DOMK,9011) trim(VERSN), RUNDAT, RUNTIM
9011 FORMAT('TERSCI Version ',A,1x,A8,1x,A8/)

  WRITE(DOMK,50) TYPDEM
50 FORMAT('DOMCNV/CHKEXT:',//'TYPDEM = "',A3,'"')

!*    Write domain SW and NE corner coordinates in both lat/lon and UTMs;
!*    include UTM zone(s) and reference datum (NADA) from ANCHORXY keyword
