    SUBROUTINE CHKEXT
!***********************************************************************
!*                CHKEXT Module of AERMAP Terrain Preprocessor
!*
!*       PURPOSE: Check to See That the Extent of the Receptors is Within
!*                the Extents of the User-Specified Domain and the Domain
!*                Itself is Within the Supplied Raw dem/user Terrain Files
!*
!*
...
!*
!*                Modified to check for the domain extent based on the
!*                UTM corner coordinates for DEM or NED files with
!*                boundaries that follow UTM lines when TYPDEM = 'UTM'.
!*                Modified debug file output to include UTM zone for
!*                receptors and sources relative to domain, Lat/Lon
!*                coordinates in both arc-sec and degrees, and column
!*                headers.  Modified for use of standard convention of
...
!*
!*       MODIFIED:   Correct error that can occur with 1-degree DEM data
!*                   when the number of sources exceeds the number of
!*                   receptors.  R.W. Brode, PES, 1/22/98
!*
!*       INPUTS:  Receptor coordinates in cartesian coords
!*
!*       OUTPUTS: Logical Flags Denoting Whether In or Out.
!*
!*       CALLED FROM:   MAIN
!***********************************************************************
...
!*    Variable Declarations
    ...
    real (kind=8) :: DOMLLSHFT_1, DOMLLSHFT_2, NADFLG
    real (kind=8) :: DOMAINXSHFT, DOMAINYSHFT
...
!*    Check to See if the Receptors are Within the User-Specified
!*    Domain after Converting Domain Extents to Lat/Long secs

!*    (If the Terrain Data is of Type 'LAT', then the Domain is in
!*     Lat/Long. Otherwise, it is in UTM Coords and needs to be Converted
!*     Into Arc Secs)
...
!*
    ...
    !*       If a Receptor is in the Domain, Set the Flag to True and keep looping
    !          no flag is set within the condition statements. However, if a
    !          condition turns out to be false, a fatal message is written.
        IF (TYPDEM == 'LAT') THEN
           IF(RLAT(IREC) >= DOMLL(1,1) .AND. &
            RLAT(IREC) <= DOMLL(1,2) .AND. &
            RLON(IREC) >= DOMLL(2,1) .AND. &
            RLON(IREC) <= DOMLL(2,3)) THEN

                CYCLE

            ELSE
            !*             Write Fatal Message, Receptor outside domain
                WRITE (DUMMY,'(I8)') IREC
                CALL ERRHDL(PATH,MODNAM,'E','300',DUMMY)
            ENDIF
                        
        ELSE IF (TYPDEM == 'UTM') THEN
            IF (ZONMIN == ZONMAX) THEN
                IF (XRECU(IREC) >= XDMIN .AND. &
                XRECU(IREC) <= XDMAX .AND. &
                YRECU(IREC) >= YDMIN .AND. &
                YRECU(IREC) <= YDMAX) THEN
                ...
                !*                Write Fatal Message, Receptor outside domain
                    ...
                    CALL ERRHDL(PATH,MODNAM,'E','300',DUMMY)
                ENDIF
            ELSE IF (IZONR(IREC) == ZONMIN) THEN
                IF (XRECU(IREC) >= XDMIN .AND. &
                YRECU(IREC) >= YDMIN .AND. &
                YRECU(IREC) <= YDMAX) THEN
                ...
                !*                Write Fatal Message, Receptor outside domain
                    ...
                    CALL ERRHDL(PATH,MODNAM,'E','300',DUMMY)
                ENDIF
            ELSE IF (IZONR(IREC) == ZONMAX) THEN
                IF (XRECU(IREC) <= XDMAX .AND. &
                YRECU(IREC) >= YDMIN .AND. &
                YRECU(IREC) <= YDMAX) THEN
...
!*
    IF (NUMSRC > 0) THEN
        ...
        !*          If a Source is in the Domain, Set the Flag to True and keep looping
            IF (TYPDEM == 'LAT') THEN
                IF(SLAT(ISRC) >= DOMLL(1,1) .AND. &
                SLAT(ISRC) <= DOMLL(1,2) .AND. &
                SLON(ISRC) >= DOMLL(2,1) .AND. &
                SLON(ISRC) <= DOMLL(2,3)) THEN

                    CYCLE
                                   
                ELSE
                !*                Write Fatal Message, Source outside domain
                    CALL ERRHDL(PATH,MODNAM,'E','305',SRCID(ISRC))
                ENDIF

            ELSE IF (TYPDEM == 'UTM') THEN
                IF (ZONMIN == ZONMAX) THEN
                    IF (XSRCU(ISRC) >= XDMIN .AND. &
                    XSRCU(ISRC) <= XDMAX .AND. &
                    YSRCU(ISRC) >= YDMIN .AND. &
                    YSRCU(ISRC) <= YDMAX) THEN
                    ...
                    !*                   Write Fatal Message, Source outside domain
                        CALL ERRHDL(PATH,MODNAM,'E','305',SRCID(ISRC))
                    ENDIF
                ELSE IF (IZONS(ISRC) == ZONMIN) THEN
                    IF (XSRCU(ISRC) >= XDMIN .AND. &
                    YSRCU(ISRC) >= YDMIN .AND. &
                    YSRCU(ISRC) <= YDMAX) THEN
                    ...
                    !*                   Write Fatal Message, Source outside domain
                        CALL ERRHDL(PATH,MODNAM,'E','305',SRCID(ISRC))
                    ENDIF
                ELSE IF (IZONS(ISRC) == ZONMAX) THEN
                    IF (XSRCU(ISRC) <= XDMAX .AND. &
                    YSRCU(ISRC) >= YDMIN .AND. &
                    YSRCU(ISRC) <= YDMAX) THEN
...
!*    Then Check to See if the Domain extents are Within the
!*    Terrain Files.  This is accomplished by assuming that if
!*    all the 4 domain corners are inside a DEM or USER Terrain
!*    File, the Entire Domain is Inside the Extent of the Files

!*    Loop Over All 4 Domain Corners

!*    Check domain:
!*
    DO IDOM = 1, 4

    !*       If this Domain Corner hasn't Been Located Yet -
        IF (DOMIN(IDOM) == 0) THEN
                     
            DO IDEM = 1, NUMDEM

            !*             If this Domain Corner is in the Lat/Long Extent of this
            !*             Terrain File, Set the Logical for this Domain Corner
            !*             to True and Quit Domain Loop
            !*
            !*             First determine whether to apply NAD shift to Domain
            !*
            ...
            !*             Apply NAD shift to DOMLL if necessary
                ...
                IF (TYPDEM == 'UTM' .AND. &
                (((IDOM == 1 .OR. IDOM == 2) .AND. &
                ZMINSHFT /= IZOND(IDEM)) &
                 .OR. &
                ((IDOM == 3 .OR. IDOM == 4) .AND. &
                ZMAXSHFT /= IZOND(IDEM)))) THEN
                !*                Domain zone doesn't match DEM zone;
                !*                Convert domain Lat/Lon to UTMs, based on DEM file NADD and DEM Zone
                ...
                ELSE IF (TYPDEM == 'UTM' .AND. &
                    ((IDOM == 1 .OR. IDOM == 2) .AND. &
                    ZMINSHFT == IZOND(IDEM))) THEN
                    ZONSHFT = ZMINSHFT
                ELSE IF (TYPDEM == 'UTM' .AND. &
                    ((IDOM == 3 .OR. IDOM == 4) .AND. &
                    ZMAXSHFT == IZOND(IDEM))) THEN
            ...
            ! ---          Output coords of northeast corner of elev file
            !              lat, lon (arc-seconds then degrees) for LAT files
            !              UTMx, UTMy, and zone for UTM files; also include NADD(IDEM)
                IF (TYPDEM == 'UTM' .AND. (IPLAN(IDEM) == 1 .AND. &
                ...
                ! ---             Output NE corner coordinates in UTMs
                ...
                ! ---             Output SW corner coordinates in UTMs
                ...
                ! ---             Output domain lat/lon (arc-seconds and degrees)
            ...
            ! ---          Check to see if domain is inside extent of elev file.
                IF (TYPDEM == 'UTM' .AND. (IPLAN(IDEM) == 1 .AND. &
                DABS(NWE_MTRS(IDEM)-SWE_MTRS(IDEM)) < &
                &                                              0.05D0*DXM(IDEM) .AND. &
                DABS(NEE_MTRS(IDEM)-SEE_MTRS(IDEM)) < &
                &                                              0.05D0*DXM(IDEM) .AND. &
                (DOMAINXSHFT >= SWE_MTRS(IDEM) .AND. &
                DOMAINXSHFT <= NEE_MTRS(IDEM) .AND. &
                DOMAINYSHFT >= SWN_MTRS(IDEM) .AND. &
                DOMAINYSHFT <= NEN_MTRS(IDEM))) &
                 .OR. &
                (DOMLLSHFT_1 >= SWLAT_ARCS(IDEM) .AND. &
                DOMLLSHFT_1 <= NELAT_ARCS(IDEM) .AND. &
                DOMLLSHFT_2 >= SWLON_ARCS(IDEM) .AND. &
                DOMLLSHFT_2 <= NELON_ARCS(IDEM))) THEN
                         
                !*                Domain Point is INSIDE the DEM file
                    DOMIN(IDOM) = 1
                ...
                !*                Domain Point is NOT inside the DEM file
                    DOMIN(IDOM) = 0
    ...
    DO IDOM = 1,4
    !*       If This Domain Point is Not Inside Any Terrain File,
    !*       Then Write an Error Message and Continue.
        IF (DOMIN(IDOM) == 0) THEN
            WRITE (DUMMY,'(I8)') IDOM
