    SUBROUTINE NEDCHK
!**********************************************************************
!*        NEDCHK Subroutine for the AERMAP Terrain Preprocessor
!*
!*        PURPOSE: Check NED file in GeoTIFF format to determine if it
!*                 meets minimum criteria to extract NED data.  Calls
!*                 Subroutine read_tifftags which returns arrays that
!*                 store all TIFF Tags, GeoKeys and their values.
!*                 Evaluates certains tags and keys for expected values.
!*
...
! tie point anchor in raster space
    real (kind=8) :: tiePtAncx   ! anchor point for tie point, x
    real (kind=8) :: tiePtAncy   ! anchor point for tie point, y
    real (kind=8) :: tiePtAncz   ! anchor point for tie point, z
...
! --- Allocate SampleFormat array
    allocate (SampleFormat(ndem))
          
! --- initialize tag and geokey counters
    tCnt = 0
    gCnt = 0

! --- initialize SampleFormat array (ndem) to default value per TIFF6.0 specs;
!     SampleFormat = 1 for unsigned integer
    SampleFormat = 1
          
! --- initialize ModelType codes to 0 (undefined)
    ModelType    = 0
    geoType      = 0
    geoDatum     = 0
    geoEllipse   = 0
...
! --- Open MAPDETAIL.OUT debug file
    open(unit=demk, file = mapdet_file, status = 'replace')
!*    Write Version Date Header to MAPDETAIL.OUT File
    WRITE(DEMK,9011) trim(VERSN), RUNDAT, RUNTIM
9011 FORMAT('TERSCI Version ',A,1x,A8,1x,A8/)

    WRITE(DEMK,*) 'From NEDCHK:',numdem

! --- Loop over NED files and process tiff tags and geokeys
    ...
    ! ---    Initialize gotTag and tNdx fields for tFlgs and gFlgs arrays
    ...
    ! ---    Generate truncated filename, without path (up to 40 characters)
        I = INDEX(DEMFIL(IDEM),'/',BACK= .TRUE. )
        J = INDEX(DEMFIL(IDEM),ACHAR(92),BACK= .TRUE. )   ! ACHAR(92) = backslash
        IF (I > 0) THEN
            IF (LEN_TRIM(DEMFIL(IDEM))-I <= 40) THEN
        ...
        ELSE IF (J > 0) THEN
            ...
            IF (LEN_TRIM(DEMFIL(IDEM))-J <= 40) THEN
                FLN(IDEM) = DEMFIL(IDEM)(J+1:LEN_TRIM(DEMFIL(IDEM)))
            ELSE
                FLN(IDEM) = DEMFIL(IDEM)(LEN_TRIM(DEMFIL(IDEM))-39: &
                LEN_TRIM(DEMFIL(IDEM)))
    ...
    ! ---    print message to map detail file for file to be read
    ...
    ! ---    process error and warning flags returned from read_tifftags;
    !        messages should have been written to debug file, no need to duplicate
    ...
    ! ---    Assign "DEMLVL" code for NED files to 5 = ' NED '
        DEMLVL(idem) = 5

    ! ---    set global byteswap flag returned from Read_TiffTags subroutine
        byteswap(idem) = swapbytes

    ! ---    Loop over tiff tags array and local tiff flags array
    !        Set flags and index refs
    ...
    ! ---    Loop over geokey array and local geokey flags array
    !        Set flags and index refs
    ...
    ! ---    process tiff tags found to collect/evaluate values
    ...
    ! ---    Special tag depencency processing
    !        Determine tiff type (strip or tile)
        ...
        ! ---       RowPerStrip not defined; default value is number of rows
            tiffType(idem) = 'strip'
            write(demk,'(a)') ' TIFF Data Organization: Strips'
    ...
    ! ---    Write sample format and bytes per sample to MAPDETAIL.OUT file
    ...
    ! ---    process GeoKeys found to collect/evaluate values
    ...
    ! loop over local geokey flags array
        ...
        ! ---                Assign ModelType code
            ModelType = gKeys(gFlgs(k)%tNdx)%intVal(1)
                                
        !                    Assign TYPDEM based on first data file
            if (idem == 1) TYPDEM = 'UTM'
                                 
        !                    Assign IPLAN code for ground planimetric reference system
            IPLAN(idem) = 1        ! UTM
        ...
        !                    Assign IPLAN code = 9 to indicate missing value
            IPLAN(idem) = 9
    ...
    ! ------ Begin processing tifTags and geoKeys to determine file type/datum,
    !        and units, and to check for completeness of file specifications.
    !        Requirements based on ModelType (1=Projection, 2=Geographic)
    ! ------
        select case(ModelType)
                 
        case(1)      ! Projected File

    ! ---       This is a projection file; first check for proper horizontal units
        if (projUntsFlg) then
    ...
    ! ---       Process projCStype codes for datum and UTM zone
    ...
    ! ---       This is a geographic file; first check for proper angular units
        if (geogUntsFlg) then
    ...
    ! ---    Processing of elevation units geoKeys and optional user-specfied
    !        elevation units and vertical scale resolution.
    !        If elevation units flag is still false, write message to debug file
        ...
        ! NED file does not include vertical elevation units geoKey (4099)
            ...
            ! pxlScalez
                ...
                ELUNIT(idem) = 2
            !              DCI (vertical resolution) already set based on pxlScalez
                write(dummy,'(i8)') idem
    ...
    ! ---    Special tag dependency processing
    !        Adjust tie points, derive corner coordinates
    !        These are dependent on the pixel scale and raster type,
    !        so first need to check for existence of pixel scale,
    !        raster type and tie points

        if (pxlSclFlg .AND. tiePntFlg .AND. RasterTypeFlg) then

        ! ---       Process GT Raster Type code; determine whether to adjust
        !           model tie point
        ! ---       Assign adjusted or unadjusted value to DMCNR array NW point
            if (PixelIsArea) then
        ...
        ! ---       Check for ground planimetric reference system (geographic or UTM)
        !           and horizontal units consistency; conversions to standard units of
        !           arc-seconds for geographic and meters for projected (UTM) files is
        !           done below.
        ! ---       Determine DMCNR coordinates and apply horizontal unit
        !           conversions, if needed
        !           Start with corner index = 2 - NW Corner (upper left corner,
        !           corresponds with model tie point, with possible adjusment to
        ! ---       pixel center)
            if (iplan(idem) == 0) then
            ! ---          Convert horizontal units to arc-seconds for geographic files
                ...
                ! ---             Write out model tie point and spatial resolution to MAPDETAIL.OUT file
                    ...
                    f11.3,2x,a6)
                ! ---             Check for unexpected combination of pxlScalez and tiePtAncz
            ...
            ! ---          Convert horizontal units to meters for UTM projected files
                ...
                ! et spatial resolution in x and y directions (meters)
        ...
        ! ----      Check for appropriate range of horizontal resolution (DXM):
        !           Geographic:  current range of standard resolutions is
        !                        1/9 arc-second to 3 arc-seconds;
        !                        warn for DXM > 3 seconds, error for DXM > 30 seconds
        !                        warn for DXM < 0.1 second
        !           Projected:   current range of standard resolutions is
        !                        3 meters to 90 meters;
        !                        warn for DXM > 100 meters, error for DXM > 1000 meters
        !                        warn for DXM < 1 meter
        ! ----
            if (IPLAN(idem) == 0) then
            ! Geographic file
        ...
        !----       Get UTM zone for file based on SW corner
            if (IPLAN(idem) == 0) then
                IZO(idem) = utm_zone((DMCNR(idem,1,1)+nudge)/3600.0D0)
                if (((DMCNR(idem,2,1)+nudge)/3600.0D0) < 0.0D0) then
                    IZO(idem) = -1*IZO(idem)
                end if
                IZOND(idem) = IZO(idem)
            end if
                        
            if (ABS(IZO(idem)) >=  1 .AND. &
            ABS(IZO(idem)) <= 60) then
    ...
    ! ---    Check for whether NADCON grid files will be needed
        IF( (NADA == 1  .OR.  NADA >= 5) .AND. &
