...
!***********************************************************************
!     AERMAP.INC
!     AERMAP Data - Array Names, Array Limits, Named Common Blocks
!                   Global Data for All Modules

!     MODIFIED - 4/24/2003 (03107)
!***********************************************************************
...
!**   Set record length for direct access elevation files, LREC_DIR
!**   LREC_DIR = 8 for 8-byte (real (kind=8)) elevation data
    INTEGER, PARAMETER :: LREC_DIR = 8

!**   Nudge factor, in arc-seconds, for nudging DEM corner coordinates
!**   inward toward the file so that converted coordinates which may
!**   be slightly over a UTM zone boundary due to precision or rounding
!**   will be assigned to the expected zone.
!**   Nudge distance of 0.03 arc-seconds is approximately 1 meter.
    real (kind=8), PARAMETER  :: nudge = 0.03D0

!**   The following array limits are allocated dynamically at runtime, based
!**   on the input data:
    INTEGER :: MAXPRF
    INTEGER :: NREC, IXM, IYM, NNET, NDEM, NSRC, NARC
        
!**   MAXPRF = Max Number of Profiles per data file
...
!**   NDEM   = Max Number of Digital Elevation MODEL(DEM) Terrain Data Files
!**   NSRC   = Max Number of Sources
!**   NARC   = Max Number of Receptor Groupings ('ARCs') for EVALCART Keyword


!***********************************************************************
!     Model Constants Initialized in sub.VARINI
!***********************************************************************

    real (kind=8) :: PI, DTORAD, RTODEG

!**   PI     = Number of radians in 180 degrees; assigned in sub.VARINI
!**            as 4.0D0*DATAN(1.0D0)
!**   DTORAD = Degrees to Radians Conversion Factor; assigned in sub.VARINI
!**            as PI/180.0D0
!**   RTODEG = Radians to Degrees Conversion Factor; assigned in sub.VARINI
!**            as 180.0D0/PI
...
!**   ISTRG    = Length of Runstream Image Character String
...
! fl 9/5/02 Add type & common for input/output used in NADCON21 as part of a
! fl        change to be sure unit numbers are assigned before the corresponding
! fl        write statements are executed.
    INTEGER, PARAMETER :: MXAREA = 8
    INTEGER :: LUIN, LUOUT, NOUT, NIN, NAPAR, LUAREA(2*MXAREA)
    INTEGER :: ILEN_PATH

! fl  Change by Russell F. Lee, under contract to Lakes Environmental Software
! fl  End of change

!**   INUNIT = Input Runstream File Unit (Initialized to 5)
!**   IOUNIT = Main Printed Output File Unit (Initialized to 6)
!**   IRUNIT = Receptor Data Output File Unit (Initialized to 8)
!**   ISUNIT = Source Data Output File Unit (Initialized to 9)
!**   IERUNT = Temporary Error/Message File Unit (Initialized to 10)
!**   IERWRT = Permanent Detailed Error/Message File Unit (Init. to 11)
!**   IDBGUNT = Read debug switches Unit (Init. to 12)
!**   INCUNT = INCLUDED File Unit (Initialized to 14)
!**   IDMUNT = DEM DATA Input File Unit (Based from unit 100)
!**   IDRUNT = DEM Direct Access File Unit to Elevation DATA (Based from unit 300)
!**   IDXUNT = DEM Index File Unit (Based from unit 500)
...
!*    Declare character variable for Index File Format, IDXFRM
    ...
    CHARACTER (LEN=200), ALLOCATABLE :: DEMFIL(:)
    CHARACTER (LEN=200), ALLOCATABLE :: DIRFIL(:)
    CHARACTER (LEN=200), ALLOCATABLE :: IDXFIL(:)
    CHARACTER (LEN=200), ALLOCATABLE :: TiffDbgFil(:)
...
!***********************************************************************
    ...
    CHARACTER ERRMSG*50, ERRCOD*3, VERSN*30
...
!*****  NOTE: RURAL and URBAN have been returned to COMMON /CNTLOG/
...
!***********************************************************************
    ...
    real    :: Spacing ! for sci_terrain
         
         
    real (kind=8) :: XINT, YINT, XDELTA, YDELTA
    real (kind=8) :: XNODE, YNODE, XDMIN     , XDMAX, YDMIN, YDMAX
    real (kind=8) :: XATERR, YATERR, XAUSER, YAUSER
          
    real (kind=8), ALLOCATABLE :: AXR(:), AYR(:)
    real (kind=8), ALLOCATABLE :: AXS(:), AYS(:)
    real (kind=8), ALLOCATABLE :: RLAT(:), RLON(:)
    real (kind=8), ALLOCATABLE :: SLAT(:), SLON(:)
    real (kind=8), ALLOCATABLE :: XRECU(:), YRECU(:)
    real (kind=8), ALLOCATABLE :: XCOORD(:,:)
    real (kind=8), ALLOCATABLE :: YCOORD(:,:)
    real (kind=8), ALLOCATABLE :: XSRCU(:), YSRCU(:)
    real (kind=8), ALLOCATABLE :: XORIG(:), YORIG(:)
    real (kind=8), ALLOCATABLE :: XRDIFS(:), YRDIFS(:)
    real (kind=8), ALLOCATABLE :: XRDIFM(:), YRDIFM(:)
    real (kind=8), ALLOCATABLE :: XSDIFS(:), YSDIFS(:)
    real (kind=8), ALLOCATABLE :: XSDIFM(:), YSDIFM(:)
    real (kind=8), ALLOCATABLE :: DATUMSHFT(:), DATUMSHFTS(:)

    real (kind=8), ALLOCATABLE :: XBASE(:), YBASE(:)
          
    real (kind=8), ALLOCATABLE :: DXM(:), DYM(:)
         
    real (kind=8) :: XPT27, YPT27, XPT83, YPT83, AMAG
    real (kind=8) :: DLOS, DLOM, DLAS, DLAM, EPS
    real (kind=8) :: XDMNDIFS, YDMNDIFS, XDMNDIFM, YDMNDIFM
    real (kind=8) :: XDMXDIFS, YDMXDIFS, XDMXDIFM, YDMXDIFM
    real (kind=8) :: XDMNSHFT, YDMNSHFT, XDMXSHFT, YDMXSHFT
    real (kind=8) :: RDIST, DOMLL(2,4)
    real (kind=8) :: X(4), Y(4)
          
    real (kind=8), ALLOCATABLE :: HC(:), AZS(:), AZELEV(:)
    real (kind=8), ALLOCATABLE :: AZFLAG(:), AZHILL(:)
    real (kind=8), ALLOCATABLE :: ZETMP1(:), ZETMP2(:)
    real (kind=8), ALLOCATABLE :: ZFTMP1(:), ZFTMP2(:)

    real (kind=8), ALLOCATABLE :: DCI(:), UserDCI(:)
...
!***********************************************************************
!     This is The Global Variable Definition Block for Receptor Network Origin
!***********************************************************************
...
!***********************************************************************
    ...
    real (kind=8), ALLOCATABLE :: SWE_MTRS(:), SWN_MTRS(:)
    real (kind=8), ALLOCATABLE :: NWE_MTRS(:), NWN_MTRS(:)
    real (kind=8), ALLOCATABLE :: NEE_MTRS(:), NEN_MTRS(:)
    real (kind=8), ALLOCATABLE :: SEE_MTRS(:), SEN_MTRS(:)
    real (kind=8), ALLOCATABLE :: SWLAT_ARCS(:),SWLON_ARCS(:)
    real (kind=8), ALLOCATABLE :: NWLAT_ARCS(:),NWLON_ARCS(:)
    real (kind=8), ALLOCATABLE :: NELAT_ARCS(:),NELON_ARCS(:)
    real (kind=8), ALLOCATABLE :: SELAT_ARCS(:),SELON_ARCS(:)
    real (kind=8), ALLOCATABLE :: SWLAT_DEGS(:),SWLON_DEGS(:)
    real (kind=8), ALLOCATABLE :: NWLAT_DEGS(:),NWLON_DEGS(:)
    real (kind=8), ALLOCATABLE :: NELAT_DEGS(:),NELON_DEGS(:)
    real (kind=8), ALLOCATABLE :: SELAT_DEGS(:),SELON_DEGS(:)
          
    INTEGER, ALLOCATABLE          :: SW_UTMZ(:), SE_UTMZ(:)
    INTEGER, ALLOCATABLE          :: NW_UTMZ(:), NE_UTMZ(:)
          
    real (kind=8), ALLOCATABLE :: ELEVMAX(:)
    real (kind=8), ALLOCATABLE :: DLTN(:), DLGE(:)
    real (kind=8), ALLOCATABLE :: MPROJ(:,:), DMCNR(:,:,:)
    real (kind=8), ALLOCATABLE :: ELEVMN(:), ELEVMX(:)
    real (kind=8), ALLOCATABLE :: CNTRC(:)
          
    real (kind=8), ALLOCATABLE :: LOCEL(:)
    real (kind=8), ALLOCATABLE :: MINEL(:), MAXEL(:)
...
! fl 9/5/02 Add I/O file units used in NADCON21 so that they can be
! fl          opened before NADCON is called
!                     LUOOUT ! Information is written to AERMAP.OUT
    DATA LUIN /35/, LUOUT / 7/, NOUT /31/, NIN /32/, NAPAR /33/
! fl  Note: LUOUT is intentionally given the value of IOUNIT (i.e., 7) so that
! fl  the grid file information can all be output to AERMAP.OUT.
! fl  Change by Russell F. Lee, under contract to Lakes Environmental Software
! fl  End of change

!**** Define Index File Format, IDXFRM
!     Read parameters are: XBASE, YBASE, NUMNODES, UTMZONE, MAXELEV
...
!*    Now assign individual error codes and messages
...
! --- The following vars are specific for processing GeoTIFF files
    ...
    real (kind=8), allocatable :: tiePtx(:)    ! upper left X coordinate NED GeoTIFF
    real (kind=8), allocatable :: tiePty(:)    ! upper left Y coordinate NED GeoTIFF
    real (kind=8), allocatable :: tiePtz(:)    ! upper left Z coordinate NED GeoTIFF
    real (kind=8), allocatable :: pxlScalex(:) ! model pixel scale X-direction
    real (kind=8), allocatable :: pxlScaley(:) ! model pixel scale Y-direction
    real (kind=8), allocatable :: pxlScalez(:) ! model pixel scale Z-direction
          
    type arrOffsets
    integer (kind=8), allocatable :: numbytes(:)
    end type
