    SUBROUTINE DEMCHK
!***********************************************************************
!*               DEMCHK Module of AERMAP Terrain Preprocessor
!*
!*       PURPOSE: Checks each DEM file for errors and identifies the
!*                file format (Continuous ASCII [no CRs no LFs],
!*                Delimited [DOS or UNIX], binary or unidentified.)
!*
...
!*
!*                Incorporated additional error checking, clarified
!*                and simplified processing to determine DEM file type,
!*                removed some redundant information from the
!*                MAPDETAIL.OUT debug file, and included allocatable
!*                arrays for number of profiles within DEM files.
!*
!*       MODIFIED BY: Roger W. Brode, U.S. EPA, OAQPS, AQMG
!*
...
!*       INPUTS:  DEM file
!*
!*       OUTPUTS: Flag indicating structure and a file, MAPDETAIL.OUT
!*                with DEM header values by file and then grouped by
!*                variable.  Variables from first three and last three
!*                records of USGS Record Type B are also printed.
!*
!*       CALLED FROM:   MAIN
!***********************************************************************
...
!*    Initialize MAXPRF variable for maximum number of DEM profiles
    MAXPRF = 0
          
!*    Assign default limit on number of characters to read at 10240,
!*    eqivalent to about 10 "records"
    MAXT = 10240
...
!*       Initialize variable for IOSTAT condition
        IOS = 0
...
!*       Read through a DEM file byte by byte recording the quantity
!*         of each ASCII code in the file.  Used in determining whether
!*         the file is delimited (UNIX or DOS) or not (continuous record).
!*         AERMAP has been designed to handle either delimited or continuous
!*         data files, defined as FT = 1 or 2, respectively. Files with invalid
!*         characters are assumed to be binary, which cannot be processed by AERMAP.
!*         FT(IDEM) = 1:  No line-feed (LF) or carriage-return (CR) (one continuous record)
!*         FT(IDEM) = 2:  Delimited file, DOS (CR and LF) or UNIX (LF with no CR);
!*                        DOS and UNIX type files are handled the same in AERMAP,
!*                        FT(IDEM) = 3 is no longer used.
!*         FT(IDEM) = 4:  Invalid file type, possible binary file
...
!*         Check for DEMCHECK switch; Exit Loop if no-check and T > MAXT
            IF ( .NOT. L_DEMCHECK(IDEM) .AND. T >= MAXT) EXIT

!*         Assign ASCII character code for this character
            K = IACHAR(ACHR)
            LL(K) = LL(K) + 1
...
!        Output read counter (T) and IOSTAT value (IOS)
...
!        End of File Structure Analysis
...
!        **************************************************************
        ...
        92 CONTINUE
!        End-of-record (EOR) encountered
          
        IF (L_DEMCHECK(IDEM) .AND. T > MAXT) THEN
!           End-of-record reached beyond MAXT, this implies a continuous record file
            FT(IDEM) = 1
        ELSE
!           End-of-record reached before MAXT, this implies DOS or UNIX file (assign 2)
            FT(IDEM) = 2
        END IF

        95 CONTINUE
!        End-of-file (EOF) branch

        CLOSE(IDMUNT(IDEM))

!        Check for evidence that file is binary
        DO K = 0, 255
            IF (LL(K) > 1) THEN
                IF (K < 32 .OR. K > 126) THEN
!              Non-standard character; set flag for "binary" file
                    CFLG(IDEM) = 'YES'
                    FT(IDEM)   = 4
                    EXIT
                END IF
        ...
        FLEN(IDEM) = T
!        File Structure Analysis Writeup
        ...
        IF (FT(IDEM) == 4) THEN
            ...
            DO K = 0, 255
                IF(K < 32) THEN
                    IF(LL(K) > 0) WRITE (DEMK,56) K, CHR(K), LL(K)
                ELSE
                    IF(LL(K) > 0) WRITE (DEMK,56) K, CHAR(K), LL(K)
                END IF
            END DO
...
! ---       Cycle to next DEM file
            CYCLE DEMLOOP
        END IF

        IF (FT(IDEM) /= 1) THEN
! ---       Record delimited file (DOS or UNIX)
            OPEN(UNIT = IDMUNT(IDEM), FILE= DEMFIL(IDEM), ERR = 96)
                        
        ELSE
! ---       Continuous record file
            OPEN(UNIT = IDMUNT(IDEM), FILE= DEMFIL(IDEM), &
            RECL = 1024, &
            ACCESS = 'DIRECT', &
            FORM ='FORMATTED', &
...
!        USGS Record Type A - header of a DEM file (first line)
...
! ---       Continuous record file
        ...
        IF (DEMLVL(IDEM) >= 1 .AND. DEMLVL(IDEM) <= 4) THEN
        ...
        ELSE
            ...
            WRITE(DEMK,*) ' '
        END IF
        IF (ELEVPAT(IDEM) >= 1 .AND. ELEVPAT(IDEM) <= 2) THEN
        ...
        ELSE
            ...
            WRITE(DEMK,*) ' '
        END IF
        IF (IPLAN(IDEM) >= 0 .AND. IPLAN(IDEM) <= 2) THEN
        ...
        IF (CUNIT(IDEM) >= 0 .AND. CUNIT(IDEM) <= 3) THEN
        ...
        ELSE
            ...
            WRITE(DEMK,*) ' '
        END IF
        IF (ELUNIT(IDEM) >= 1 .AND. ELUNIT(IDEM) <= 2) THEN
        ...
        IF (DEMERR) THEN
! ---       Error(s) with processing Record A, cycle to next file
            CYCLE DEMLOOP
        END IF
        ...
        IF (LPINT(IDEM) >= 0 .AND. LPINT(IDEM) <= 2) THEN
        ...
        IF (SUSF(IDEM) >= 0 .AND. SUSF(IDEM) <= 3) THEN
        ...
        ELSE
            ...
            WRITE(DEMK,*) ' '
        END IF
        IF (VDAT(IDEM) >= 0 .AND. VDAT(IDEM) <= 3) THEN
        ...
        ELSE
            ...
            WRITE(DEMK,*) ' '
        END IF
        IF (NADD(IDEM) >= 0 .AND. NADD(IDEM) <= 7) THEN
...
! ---    Check for agreement between IPLAN, CUNIT, and IZO
!        and generate error and warning messages appropriately
!        When IPLAN = 0 (geographic), CUNIT = 3 (arc-seconds), (IZO should = 0)
!        When IPLAN = 1 (UTM), CUNIT = 2 (meters), IZO = integer (1-60)
!        ERROR: Disagreement btw IPLAN and CUNIT
!        ERROR: IPLAN = 1 (UTM), CUNIT = 2 (meters), IZO = 0
!        WARNING: IPLAN = 0 (geographic), CUNIT = 3 (arc-seconds), IZO = non-zero
!                 (IZO will be reset based on SW corner coordinates)
                 
        IF (IPLAN(IDEM) == 0) THEN
! Coordinate type = Geographic lat/lon
            IF (CUNIT(IDEM) /= 3) THEN
! Coordinate units NOT EQUAL arc-seconds
                DEMERR = .TRUE. 
                WRITE (DUMMY,'(I8)') IDEM
                CALL ERRHDL(PATH,MODNAM,'E','445',DUMMY)
                            
            ELSE IF (CUNIT(IDEM) == 3 .AND. IZO(IDEM) /= 0) THEN
...
! Coordinate units NOT EQUAL meters
            ...
            ELSE IF (CUNIT(IDEM) == 2 .AND. &
                (ABS(IZO(IDEM)) < 1 .OR. ABS(IZO(IDEM)) > 60)) THEN
! nits = meters, utm zone not in range
                DEMERR = .TRUE. 
                WRITE (DUMMY,'(I8)') IDEM
...
!*       Check Number of Profiles and reset MAXPRF if needed
        IF (NPROF(IDEM) > MAXPRF) THEN
            MAXPRF = NPROF(IDEM)
        END IF
...
!*    Allocate arrays based on MAXPRF
